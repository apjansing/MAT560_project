...
def findEigenValuesVectorsAndPlot(filename):
    ...
    for i in reversed(range(eigensolver.get_number_converged())):
        ...
        # save vectors
        with open('eigen/%s/vectors/%s%d_%04d.txt' % (baseName, baseName, m, i), 'w') as f:
            vec = formatVector(str(rx.vec().getArray())) 
            f.write(vec)
        
        u.vector()[:] = rx
        plot(u.sub(0))
        # save images
        pylab.savefig('eigen/%s/images/%s%d_%04d.png' % (baseName, baseName, m, i), bbox_inches='tight')
